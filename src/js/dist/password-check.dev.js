...
var Language = document.currentScript.getAttribute('lang') ? document.currentScript.getAttribute('lang') : 'Nl';

var PasswordCheck = function PasswordCheck(CheckPassword, Element, options, config) {
  ...
  */
  ...
  input_strength_checker = function input_strength_checker(input) {
    /* --- Check letters --- */
    // Validate capital letters
    if (CapitalLength > 0) {
      // Check if CapitalLength is needed
      MaxStrength++;
    ...
    if (LowerLength > 0) {
      // Check if LowerLength is needed
      MaxStrength++;
    ...
    if (SpecialLength > 0) {
      // Check if SpecialLength is needed
      MaxStrength++;
    ...
    if (NummbersLength > 0) {
      // Check if NummbersLength is needed
      MaxStrength++;
  ...
  password_strength = function password_strength(MaxStrength, PasswordStrength, Invalid, Input) {
    // Strong
  ...
  password_response = function password_response(Strength) {
    // Enable button 
  ...
  repeat_password = function repeat_password() {
    jQuery('[' + RepeatPasswordField + '="true"]').show(); // Create field
  ...
  display_response = function display_response(Strength) {
    if (Type == 'tooltip') {
      if (!jQuery('.' + TooltipUuid).length) {
        ...
        if (PasswordLength > 0) {
          TooltipContent += '<span class="PasswordLength">' + FirsWord + PasswordLength + (Language == 'Nl' ? ' karakters' : ' characters') + ' </span>';
        }

        if (NummbersLength > 0) {
          TooltipContent += '<span class="NummbersLength">' + FirsWord + NummbersLength + (Language == 'Nl' ? ' nummers' : ' numbers') + '</span>';
        }

        if (SpecialLength > 0) {
          TooltipContent += '<span class="SpecialLength">' + FirsWord + SpecialLength + (Language == 'Nl' ? ' speciale karakters' : 'special characters') + '</span>';
        }

        if (CapitalLength > 0) {
          TooltipContent += '<span class="CapitalLength">' + FirsWord + CapitalLength + (Language == 'Nl' ? ' hoofdletters' : ' uppercase letters') + '</span>';
        }

        if (LowerLength > 0) {
          TooltipContent += '<span class="LowerLength">' + FirsWord + LowerLength + (Language == 'Nl' ? ' kleine letters' : ' lowercase letters') + '</span>';
        }
  ...
  */
  ...
  if (CheckPassword == true) {
    ...
    SimplePasswordChecker = Element.attr('checker-SimplePasswordChecker'); // Tooltip 

    TooltipUuid = Element.attr('checker-TooltipUuid');
    TooltipTitle = Element.attr('checker-TooltipTitle');
    input = Element.val(); // Check for Simple password 
