...
if ( defined( 'ABSPATH' ) && ! class_exists( 'sx_Admin_Login' ) ) {
	...
	class sx_Admin_Login {
		...
		private function new_login_slug() {
			if (
				( $slug = get_option( 'rwl_page' ) ) || (
					...
					( $slug = get_site_option( 'rwl_page', $this->NewUrl ) )
				) ||
				( $slug = $this->NewUrl )
			) {
				return $slug;
		...
		public function __construct() {
			...
			$this->NewUrl = $db->query('SELECT * FROM `'.$sx_database_options.'` WHERE `slug` = ?','LoginUrl')->fetchArray()['value'];
		...
		public function wpmu_options() {
			...
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					...
					$out .= '<td><input id="rwl-page-input" type="text" name="rwl_page" value="' . get_site_option( 'rwl_page', $this->NewUrl )  . '"></td>';
				$out .= '</tr>';
			$out .= '</table>';
		...
		public function admin_init() {
			...
			if( isset($_POST['permalink_structure']) && isset( $_POST['rwl_page'] ) ){
				...
				! in_array( $rwl_page, $this->forbidden_slugs() )
					...
					if ( is_multisite() && $rwl_page === get_site_option( 'rwl_page', $this->NewUrl ) ) {
						delete_option( 'rwl_page' );
					} else {
		...
		public function welcome_email( $value ) {
			return $value = str_replace( 'wp-login.php', trailingslashit( get_site_option( 'rwl_page', $this->NewUrl ) ), $value );
		}
